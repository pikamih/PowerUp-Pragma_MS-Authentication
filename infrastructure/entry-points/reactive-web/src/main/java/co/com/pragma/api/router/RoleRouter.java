...
public class RoleRouter {
    ...
    public RouterFunction<?> route(RoleHandler handler) {
        return RouterFunctions
                // Crear un rol
                .route(RequestPredicates.POST("/api/v1/roles"), handler::createRole)
                // Obtener un rol por ID
                .andRoute(RequestPredicates.GET("/api/v1/roles/{id}"), handler::getRoleById)
                // Listar todos los roles
                .andRoute(RequestPredicates.GET("/api/v1/roles"), handler::listRoles)
                // Actualizar un rol
